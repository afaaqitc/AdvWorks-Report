...
mf_dt<-
as.data.frame(mf) %>%  rename_with(.cols =c(1:5) ,.fn = toupper) %>% datatable(class="hover row-border", 
extensions = list(Buttons=TRUE),options = list(dom="Bfrtip",buttons=c("csv","excel","pdf"),
columnDefs=list(list(className="dt-center",targets=c(1:5))))) %>% formatCurrency(columns =c(1:5),digits = 2)
...
wfd_dt<-
wfd %>% rename_with(.cols =c(1:6) ,.fn = toupper) %>% datatable(class="hover row-border", 
extensions = list(Buttons=TRUE),options = list(dom="Bfrtip",buttons=c("csv","excel","pdf"),
columnDefs=list(list(className="dt-center",targets=c(1:6))))) %>% formatCurrency(columns = c(2:6),digits = 2)
